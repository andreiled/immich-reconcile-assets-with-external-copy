...
1.  At the same time, I like the convenience of casually browsing and sharing photos using Immich frontend
    as well as more advanced features (e.g. duplicates detection and face recognition).
...
This script addresses that last problem.
...
1.  Create a new API key for each user with the following permissions: `asset.read`, `asset.update`, `asset.delete`, `duplicate.read`.
1.  Create a new `user_libraries.json` file in the same directory (i.e. where the `docker-compose.yaml` file is) with the following content:
    ...
    _Note_: repeat the `{ ... }` part (separated by a comma) for each user.

1.  Create a new `/etc/cron.d/immich` file with the following content, owned by the `root` (need `sudo` for this):
    ...
    0 1 * * *      root    cd IMMICH_DOCKER_COMPOSE_DIR; docker compose run reconcile-assets-with-external-copy >> "/var/log/immich/$( date '+\%Y-\%m' ).reconcile-assets-with-external-copy.log" 2>&1
    #
    ```

    _Note_:
    1.  Replace `IMMICH_DOCKER_COMPOSE_DIR` with a full path to the directory containing the `docker-compose.yaml` file (same as above).
    1.  Tweak the log file name to your liking and pre-create the directory for log files in advance (e.g. `sudo mkdir -p /var/log/immich`).
